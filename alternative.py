    ...
    def play(self, *, debug=False) -> int:  #type: ignore
        ...
        while (
            ...
            if len(hands[current_player]) == 0 and last_action is not None: 
                pass
                #PUT MORE WINNING LOGIC HERE
            current_action = self._players[current_player].take_turn(tuple(hands[current_player]), len(self._players), self.RANKS[current_rank], last_action
            )   #NOT SURE IF THIS IS CORRECT
            if current_action is None or len(current_action) = 0:
                #player has performed an illegal action, but I dont know how to penalize this yet  
                pass
